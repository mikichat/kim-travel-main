...
function initializeEventListeners() {
    ...
    // Add buttons
    document.getElementById('btnAddDate').addEventListener('click', addDepartureDate);
    document.getElementById('btnAddHotel').addEventListener('click', addHotel);
    document.getElementById('btnAddItinerary').addEventListener('click', addItinerary);
    document.getElementById('btnAddOptional').addEventListener('click', addOptionalTour);
    ...
    document.getElementById('btnDownloadSample').addEventListener('click', downloadSampleExcel);
    document.getElementById('btnImportExcel').addEventListener('click', () => {
        document.getElementById('excelFileInput').click();
    });
    document.getElementById('excelFileInput').addEventListener('change', importFromExcel);
    document.getElementById('btnExportData').addEventListener('click', exportToExcel);
    ...
    document.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('remove-hotel') || e.target.closest('.remove-hotel')) {
        ...
        if (e.target.classList.contains('remove-optional') || e.target.closest('.remove-optional')) {
...
function collectDepartureDates() {
    const items = document.querySelectorAll('#departureDatesList .departure-item');
    const dates = [];
    items.forEach(item => {
...
function collectOptionalTours() {
    const items = document.querySelectorAll('#optionalToursList .optional-tour-item');
    const tours = [];
    items.forEach(item => {
...
function loadSavedPackages() {
    ...
    if (packages.length === 0) {
        container.innerHTML = '<div class="empty-message">저장된 패키지가 없습니다.</div>';
        return;
    }
    ...
    container.innerHTML = packages.map(pkg => `
        <div class="package-card" data-id="${pkg.id}">
            <div class="package-header">
                <h3>${pkg.basicInfo.packageName}</h3>
                <span class="package-code">${pkg.basicInfo.packageCode}</span>
            </div>
            <div class="package-info">
                <div class="info-item">
                    <i class="fas fa-map-marker-alt"></i>
                    <span>${pkg.basicInfo.destination}</span>
                </div>
                <div class="info-item">
                    <i class="fas fa-clock"></i>
                    <span>${pkg.basicInfo.duration}</span>
                </div>
                <div class="info-item">
                    <i class="fas fa-plane"></i>
                    <span>${pkg.basicInfo.airline}</span>
                </div>
        ...
        <div class="package-detail">
            <section>
                ...
                <table class="detail-table">
                    <tr><th>상품코드</th><td>${pkg.basicInfo.packageCode}</td></tr>
                    <tr><th>상품명</th><td>${pkg.basicInfo.packageName}</td></tr>
                    <tr><th>목적지</th><td>${pkg.basicInfo.destination}</td></tr>
                    <tr><th>여행기간</th><td>${pkg.basicInfo.duration}</td></tr>
                    <tr><th>출발도시</th><td>${pkg.basicInfo.departureCity}</td></tr>
                    <tr><th>항공사</th><td>${pkg.basicInfo.airline}</td></tr>
                </table>
            </section>
            ...
            <section>
                ...
                <div class="departure-dates-list">
                    ${pkg.departureDates.map(d => `
                        <div class="date-badge">${d.date} (잔여: ${d.seats}석)</div>
                    `).join('')}
                </div>
            </section>
            ` : ''}
            ...
            <section>
                ...
                <ul class="detail-list">
                    ${pkg.hotels.map(h => `
                        <li><strong>${h.name}</strong> (${h.rating}) - ${h.location}</li>
                    `).join('')}
                </ul>
            </section>
            ` : ''}
...
function editPackage(id) {
    ...
    document.getElementById('singleSupplement').value = pkg.pricing.singleSupplement;
    ...
    pkg.departureDates.forEach(date => {
        ...
        item.className = 'departure-item';
        item.innerHTML = `
            <input type="date" class="departure-date" value="${date.date}">
            <input type="number" class="available-seats" value="${date.seats}" min="0">
            <button type="button" class="btn-icon btn-danger remove-date">
                <i class="fas fa-times"></i>
            </button>
        `;
    ...
    pkg.hotels.forEach(hotel => {
        ...
        item.innerHTML = `
            <div class="form-grid">
                <div class="form-group">
                    <label>호텔명</label>
                    <input type="text" class="hotel-name" value="${hotel.name}">
                </div>
                <div class="form-group">
                    <label>등급</label>
                    <select class="hotel-rating">
                ...
                <div class="form-group">
                    <label>지역</label>
                    <input type="text" class="hotel-location" value="${hotel.location}">
                </div>
            </div>
            <button type="button" class="btn-icon btn-danger remove-hotel">
                <i class="fas fa-times"></i>
    ...
    pkg.optionalTours.forEach(tour => {
        ...
        item.innerHTML = `
            <div class="form-grid">
                <div class="form-group">
                    <label>관광지명</label>
                    <input type="text" class="optional-name" value="${tour.name}">
                </div>
                <div class="form-group">
                    <label>가격</label>
                    <input type="number" class="optional-price" value="${tour.price}" min="0">
                </div>
                <div class="form-group full-width">
                    <label>설명</label>
                    <textarea class="optional-description" rows="2">${tour.description}</textarea>
                </div>
            </div>
            <button type="button" class="btn-icon btn-danger remove-optional">
                <i class="fas fa-times"></i>
